...
export function AccountingShell({ children, userName = 'My account' }: { children: ReactNode; userName?: string }) {
  ...
  return <div className="ubb-accounting ubb-shell">
    <header className="ubb-appbar">
      ...
      <Link to="/settings" className="ubb-avatar" aria-label={`Account settings for ${userName}`} title={userName}>{initials}</Link>
    </header>
    <dialog ref={drawer} className="ubb-mobile-drawer" aria-label="Accounting navigation" onClick={event => { if (event.target === event.currentTarget) closeMenu() }}><div className="ubb-drawer-content"><button className="ubb-drawer-close" type="button" onClick={closeMenu} aria-label="Close navigation"><X size={22} /></button><Link className="ubb-brand" to="/accounting" onClick={closeMenu}>UBB Accounting</Link><nav className="ubb-navigation">{accountingModules.map(([id, label, Icon]) => <NavLink key={id} to={`/accounting/${id}`} onClick={closeMenu} className={`ubb-nav-item ${section === id ? 'is-active' : ''}`}><Icon size={19} />{label}</NavLink>)}</nav><Link className="ubb-back" to="/dashboard" onClick={closeMenu}><ArrowLeft size={16} />Back to TaxPhil</Link></div></dialog>
    ...
    <div className="ubb-main-column"><main id="accounting-content" className="ubb-main">{children}</main><footer className="ubb-footer"><Link to="/dashboard">TaxPhil · UBB Accounting</Link><span>{company?.membership ? `${company.membership.companyCode} · ${roleLabel(company.membership.role)} · PHP` : 'Philippines · PHP'} <span aria-hidden="true">·</span> {company ? 'Shared company books' : 'Isolated browser preview'}</span></footer></div>
  </div>
}
